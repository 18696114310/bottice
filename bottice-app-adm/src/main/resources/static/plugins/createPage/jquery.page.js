...
(function($){
    ...
    Page.prototype.updateHtml = function (pageCount) {
        var args=this.args;
        var obj=this.obj;
        if(args.current>pageCount||args.current<1)return;
        obj.empty();
        ...
        var start = args.current>args.showNear+2?args.current-args.showNear:2,end = args.current+args.showNear>=pageCount?pageCount-1:args.current+args.showNear;

        if(args.pageSwap){
            var dstart=args.current-args.showNear-2;
            var dend=pageCount-1-args.current-args.showNear;
            if(dstart<1&&dend>1){
                end+=Math.min(dend,Math.abs(dstart-1));
            }else if(dstart>1&&dend<1){
                start-=Math.min(dstart,Math.abs(dend-1));
        ...
        if(args.current + 1 + args.showNear < pageCount){
            obj.append('<span class="hiding">...</span>');
        }

        if(args.current != pageCount&&pageCount!=1){
            obj.append('<a href="javascript:;" class="tcdNumber">'+pageCount+'</a>');
        }else if(args.current== pageCount&&pageCount!=1){
            obj.append('<span class="current">'+pageCount+'</span>');
        }
        //下一页
        if(args.showNext){
            if(args.current== pageCount||pageCount==1){
        ...
        if(args.showSumNum){
            obj.append('<span class="pagecount">共'+pageCount+'页</span>');
        }
        //跳转页码
        if(args.showTurn){
            obj.append('<span class="countYe">到第<input type="text" maxlength='+pageCount.toString().length+'>页<a href="javascript:;" class="turndown">确定</a></span>');
        }
        this.setStyle();
    }
	//添加样式
	Page.prototype.setStyle=function(){
		...
		this.obj.find('input').css({'outline':'none','border':'1px solid #ddd','height':40+'px','line-height':40+'px','width':'40px',margin: '0 5px','text-align':'center'});
		
		
		this.obj.find('.tcdNumber').css({'width':40+'px','height':40+'px','line-height':40+'px'});
		this.obj.find('.hiding').css({'width':42+'px','height':42+'px','border':'1px solid #dedfe0','color':'#878787 ','margin-right':'18px','background-color':'#fff','line-height':'32px'});
