$(function () {
    $("#jqGrid").jqGrid({
        ...
        datatype: "json",
        colModel: [
			{ label: '编码', name: 'code', index: 'code', width: 96 , sortable:false, formatter: function(value, options, row)
				{
					if(null === value || undefined === value) return "";
					
					return "<a href='dictinfo.html?gid=" + row.id + "'>" + value +  "</a>";
				}
			},
			{ label: '名称', name: 'name', index: 'name', width: 120 },	
			{ label: '备注', name: 'remarks', index: 'remarks', width: 136 },
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.dto.id == null ? baseURL + "sys/dict/save" : baseURL + "sys/dict/update";
			$.ajax({
				type: "POST",
			    url: url,
